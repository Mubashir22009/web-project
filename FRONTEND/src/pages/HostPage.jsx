...
function HostPage() {
  ...
  useEffect(() => {
    ...
    const fetchInitListings = async () => {
      ...
      try {
        ...
        if (token) {
          const response = await axios.get("http://localhost:8000/api/host/listings", {
            headers: {
    ...
    const fetchInitBookings = async () => {
      ...
      try {
        const token = localStorage.getItem("token");
        if (token) {
          const response = await axios.get("http://localhost:8000/api/host/bookings", {
            headers: {
              Authorization: `Bearer ${token}`,
            }
          });
  ...
  const deleteListing = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      if (token) {
        const response = await axios.delete(`http://localhost:8000/api/host/listings/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          }
        });
